...
p <- ggplot(datasaurus2, aes(x = x, y = y, color = x)) +
  ...
  theme(legend.position = 'none',
        axis.text = element_text(color = "white", family = "Neuton", size = 11),
        axis.title = element_blank(),
        plot.title = element_text(color = "white", family = "Lobster", size = 40, hjust = 0.5, margin = margin(10, 0, 0, 0)),
        plot.subtitle = element_text(color = "white", family = "Neuton", size = 14, hjust = 0.5, margin = margin(10, 5, 25, 5)),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        strip.background = element_blank(), 
        strip.placement = "outside",
        strip.text = element_text(color = "white", family = "Neuton", size = 12),
        plot.background = element_rect(fill = "#111111"),
        plot.margin = unit(c(1, 2, 2, 2), "lines")) +
  labs(title = "Stats Only Reveal Part of the Story",
...
animate(p2, nframes = 40, fps = 5, end_pause = 5, rewind = TRUE, width = 900, height = 900, type = "cairo")
